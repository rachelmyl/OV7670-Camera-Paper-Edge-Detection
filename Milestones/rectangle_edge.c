...
void detect_and_draw_rectangle_edges(uint8_t *original, uint8_t *edge_image, int width, int height) {
    ...
    for (int y = 0; y < height / 2; y++) {
        ...
        for (int x = 0; x < width; x++) {
            if (edge_image[y * width + x] > 128) edge_count++;
        }
        if (edge_count > width / 4) { // ??25%??????
            top_y = y;
            break;
        }
    }
    ...
    // ?????????,???????
    if (top_y != -1 && bottom_y != -1 && left_x != -1 && right_x != -1 &&
        (bottom_y - top_y > 10) && (right_x - left_x > 10)) {

        // ????????????
        for (int y = top_y; y <= bottom_y; y++) {
            for (int x = left_x; x <= right_x; x++) {
                int idx = y * width + x;
                if (edge_image[idx] > 128) {
                    original[idx] = 0;
                }
            }
        }
...
int main(void)
{
  ...
  {	
    ...
    for (int i = 0; i < (IMG_ROWS * IMG_COLS); i++) {
		...
		detect_and_draw_rectangle_edges(old_snapshot_buff, edge_image, IMG_COLS, IMG_ROWS);
